...
class NewtonInterpolator:
    ...
    def reform_separated_differences_table(self, function_table):
        self.table = [[0.0] * (len(function_table) + 1) for _ in range(len(function_table) * 2 - 1)]
        for i in range(0, len(self.table), 2):
            self.table[i][0] = function_table[i // 2][0]
            self.table[i][1] = function_table[i // 2][1]
